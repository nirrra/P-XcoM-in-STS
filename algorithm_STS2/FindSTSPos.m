% STS2信号分段，并标注每段的状态 
function [posSTS,dataStatus] = FindSTSPos(grfHip,grfHip_F,grfPlantar,grfPlantar_F,showFig)
    if nargin<5, showFig = false; end
    %% 说明 
    % 各个特征点说明
    %     站起
...
%             posSeatOn = [posSeatOn,i-3];
        if range(grfHip.y(i-1:i+19)) < 15 && range(grfHip.y(i:i+20)) >= 15 && grfHip.y(i+20)>20 && max(grfHip.y(i-20:i))<50
            posSeatOn = [posSeatOn,i];
        end
    end
    posSeatOn = DeleteLater(posSeatOn,100);
...
%                 max(abs(grfPlantar_F.z(i-9:i)-grfPlantar_F.z(i-10:i-1)))<1
    ...
    posSeatOn_F = DeleteLater(posSeatOn_F,100);
...
%     hold off;
    ...
    for i = 1+20:length(grfHip.z)
        if grfHip.z(i)<2 && min(grfHip.z(i-5:i-1))>2 && max(grfHip.z(i-10:i))>10
            posSeatOff = [posSeatOff,i];
        end
        if grfHip_F.z(i)<2 && min(grfHip_F.z(i-5:i-1))>2 && max(grfHip_F.z(i-20:i))>100
            posSeatOff_F = [posSeatOff_F,i];
        end
    end
    posSeatOff = DeleteLater(posSeatOff,100);
    posSeatOff_F = DeleteLater(posSeatOff_F,100);
...
%     legend('测力台','阵列');
    ...
    for i = 1+20:length(grfPlantar_F.z)-50
        if grfPlantar_F.z(i)>400 && grfPlantar_F.z(i)-grfPlantar_F.z(i-20)>30 && range(grfPlantar_F.z(i:i+5))<5
            posSeatEnd_F = [posSeatEnd_F,i];
        end
    end
    posSeatEnd_F = DeleteFormer(posSeatEnd_F,100);
    ...
    posSeatEnd = DeleteFormer(posSeatEnd,100);
...
%         if grfPlantar.z(i)>400 && max(grfPlantar.z(i+1:i+5)-grfPlantar.z(i:i+4))<=0 && grfPlantar.z(i)-grfPlantar.z(i-1)>0 && ...
        if grfPlantar.z(i)>400 && max(grfPlantar.z(i+1:i+5)-grfPlantar.z(i:i+4))<=0 && ...
                range(grfPlantar.z(i-50:i))<20 && range(grfPlantar.z(i:i+20))>20
            posStandOn = [posStandOn,i];
        end
    end
    
    for i = 1+20:length(grfPlantar_F.z)-20
        if grfPlantar_F.z(i)>400 && max(grfPlantar_F.z(i+1:i+5)-grfPlantar_F.z(i:i+4))<=0 && ...
                range(grfPlantar_F.z(i-20:i))<20 && grfPlantar_F.z(i)-min(grfPlantar_F.z(i:i+20))>20
...
%     posStandOff = DeleteFormer(posStandOff,100);
%     posStandOff_F = DeleteFormer(posStandOff_F,100);
...
%     legend('测力台','阵列');
    ...
    posStandEnd_F = DeleteFormer(posStandEnd_F,100);
    ...
    posStandEnd = DeleteFormer(posStandEnd,100);
...
%     posStandOn = posStandOn_F; posStandOff = posStandOff_F; posStandEnd = posStandEnd_F;
    ...
    aux = posSeatEndM-posSeatOnM>200;
    posSeatOnM(aux) = []; posSeatOffM(aux) = []; posSeatEndM(aux) = [];
    aux = posStandEndM-posStandOnM>200;
    posStandOnM(aux) = []; posStandOffM(aux) = []; posStandEndM(aux) = [];
    ...
    if showFig
        ...
        plot(grfPlantar_F.z);
        plot(grfHip_F.z);
%         ShowSTS(posSeatOnM,posSeatOffM,grfPlantar_F.z,'r');
%         ShowSTS(posSeatOffM,posSeatEndM,grfPlantar_F.z,'g');
%         ShowSTS(posStandOnM,posStandOffM,grfPlantar_F.z,'b');
%         ShowSTS(posStandOffM,posStandEndM,grfPlantar_F.z,'y');
%         aux = find(dataStatus==1); plot(aux,grfPlantar_F.z(aux),'k*','MarkerSize',3);
    ...
    %% 保存结果
    ShowGroup(posStandOn);
    ShowGroup(posStandEnd);
    ...
    posSTS.seatOnM = posSeatOnM;
    posSTS.seatOffM = posSeatOffM;
    posSTS.seatEndM = posSeatEndM;
    posSTS.standOnM = posStandOnM;
    posSTS.standOffM = posStandOffM;
    posSTS.standEndM = posStandEndM;
end

function [] = ShowGroup(group)
    str = '';
    for i = 1:length(group)
        str = [str,', ',num2str(group(i))];
    end
    disp(str(3:end));
end
